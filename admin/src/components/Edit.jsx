...
export default function Edit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Fix for backend validation
      if (updatedData.hasSizes && updatedData.sizes.length === 0) {
        updatedData.hasSizes = false;
      }

      const res = await axios.put(`${backendUrl}/product/${productId}`, updatedData);
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setFormData(prev => ({
                  ...prev,
                  hasSizes: checked,
                  sizes: checked ? (prev.sizes.length > 0 ? prev.sizes : ['']) : []
                }));
              }}
            />
            يحتوي على مقاسات
